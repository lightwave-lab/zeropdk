...
def patch_simple_polygon(backend):
    class _SimplePolygon(backend.SimplePolygon):
                ...
                if left_most.x < x_bounds[0]:
                    ...
                    if right_most.x > x_bounds[0]:
                        ...
                        if y_bounds[0] < y_intersect and y_bounds[1] > y_intersect:
                            return backend.Point(float(x_bounds[0]), float(y_intersect))
                return None
        ...
        def moved(self, dx_or_dpoint, dy=None):
            if isinstance(dx_or_dpoint, (backend.Point, backend.Vector)):
    ...
    backend.SimplePolygon = _SimplePolygon
