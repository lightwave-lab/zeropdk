...
def sample_function(
    func: Callable,
    points: Union[Sequence[float], npt.NDArray[np.float_]],
    tol=0.001,
    min_points=8,
    max_level=16,
    sample_transform: Optional[Callable[[Any, T], T]] = None,
    ...
    func : callable
        ...
        so that if x has shape (N,), then func(x) should have shape (..., N).
...
def _sample_function(
    ...
    # x_c should be midpoints between x_a and x_b
    x_c = 0.5 * (x_a + x_b)
    y_c = np.atleast_2d(func(x_c))

    # Concatenate midpoints
    # points should have shape (N,)
    # x_c should have shape (N-1,)
    x_2: npt.NDArray[np.float_] = np.r_[points, x_c]
    # values should have shape (2, N)
    # y_c should have shape (2, N-1)
    y_2: npt.NDArray[np.float_] = np.r_["-1", values, y_c]
